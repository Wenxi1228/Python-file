...
def Transfer_Data_to_SOC(fhandle,mode,head,string,byte_length):   
    ...
    if(len(User_data)>1 and User_data[1]&0xf0==0x80 and User_data[0]!=0x36):
        ...
        while (recv_data[offset+12:offset+15] == bytes([0x7f, 0x31, 0x78])) or (recv_data[offset+12:offset+15] == bytes([0x7f, 0x2e, 0x78])) \
                or (recv_data[offset+12:offset+15] == bytes([0x7f, 0x27, 0x78])) or (recv_data[offset+12:offset+15] == bytes([0x7f, 0x22, 0x78])) \
                or (recv_data[offset+12:offset+15] == bytes([0x7f, 0x14, 0x78])) or (recv_data[offset+12:offset+15] == bytes([0x7f, 0x11, 0x78]))\
