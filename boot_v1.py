...
Source_address=[0x0E, 0x80]  #Tester
# Source_address = [0x10, 0x62]  # GW
...
def CalcKeyFromSeed_BOOT_HZ(seedlist):
    ...
    for jj in range(iterations):
        wTemp = int((wLastSeed ^ 0x40000000) / 0x40000000) ^ int((wLastSeed & 0x01000000) / 0x01000000) ^ int(
            (wLastSeed & 0x1000) / 0x1000) ^ int((wLastSeed & 0x04) / 0x04)
...
def Transfer_Data_to_SOC(fhandle, mode, head, string, byte_length):
    ...
    if (len(User_data) > 1 and User_data[1] & 0xf0 == 0x80 and User_data[0] != 0x36):
        ...
        # print(recv_data)
        if (recv_data.find(bytes([0x02, 0xfd, 0x80, 0x02])) != -1):
        ...
        if (recv_data.find(bytes([0x02, 0xfd, 0x80, 0x01]), 13, 18) != -1):
            offset = 13
        elif (recv_data.find(bytes([0x02, 0xfd, 0x80, 0x01]), 15, 20) != -1):
            offset = 15
            flag_error = 1
            recv_data = recv_data[27:]
        elif (recv_data.find(bytes([0x02, 0xfd, 0x80, 0x01]), 0, 5) != -1):
            offset = 0
            flag_error = 1
        ...
        while (recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x31, 0x78])) or (
                recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x2e, 0x78])) \
                or (recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x27, 0x78])) or (
                recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x22, 0x78])) \
                or (recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x14, 0x78])) or (
                recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x11, 0x78])) \
                or (recv_data[offset + 12:offset + 15] == bytes([0x7f, 0x37, 0x78])):
            soc.settimeout(4)
            recv_data = soc.recv(1024)
        ...
        if (recv_data_new == list_start and len(recv_data) == byte_length):
            fprint(fhandle, "DOIP UDS Phy Response Pass,Expect Response Data:[%s],Real Response Data:[%s]" % (
            string, receive_data))
            if (len(User_data) == 3):
            ...
            fprint(fhandle, "DOIP UDS Phy Response Fail,Expect Response Data:[%s],Real Response Data:[%s]" % (
            string, receive_data))
            if (len(User_data) == 3):
                if (User_data[1] == 0x01 and User_data[2] == 0x07):
                    return -1
...
def UserData_36(fhandle, user_data, Flist):
    ...
    # fprint(Flist)
    offset_36 = 0
    MaxDataLen = (recv_data[-3] << 8) + recv_data[-2] - 2
...
def UDPreq_vehicle(fhandle):
    ...
    time.sleep(1)
    # send_list=[0x02, 0xfd, 0x00, 0x03]+[0]*3+[0x11,0x11,0x11,0x22,0x22,0x33,0x33,0x44,0x44,0x55,0x55,0x66,0x66,0x77,0x77,0x88,0x88,0x99]
...
if __name__ == "__main__":
    ...
    folder = os.getcwd() + '\\Bootloader_log'
    print(folder)
    if not os.path.exists(folder):
        os.makedirs(folder)
    fhandle = Recodelog()
    with open('fdir.txt', mode='r') as f:
        for i in f:
            fdir += i
        print(fdir)
        MD5list = Calculate_MD5(fdir)
        Flist = ParseFile(fdir)
    # Flist=ParseFile("新建文件夹 - 副本.bin")
    ...
    while (1):
        ...
        User_data = USERDATA[index]
        if (Transfer_Data_to_SOC(fhandle, "phy", DoIP_header,
                                 "0x62 0xF1 0xC0 0x30 0x33 0x2E 0x30 0x30 0x2E 0x31 0x30",
                                 11) == 0):  # 22 F1 C0
            #break
            pass
        index += 1
        ...
        User_data = USERDATA[index]
        if (Transfer_Data_to_SOC(fhandle, "fun", DoIP_header, "0x10", 9) == 0):  # 10 83
            break
        index += 1
        # fprint(index)
